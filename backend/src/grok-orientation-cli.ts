...
if (!GROQ_API_KEY) {
  console.error("[erreur] clé GROQ_API_KEY manquante. Placez-la dans .env ou export GROQ_API_KEY=...");
  process.exit(1);
}
...
async function obtenirRecommandation(donneesUtilisateur: any) {
  try {
...
async function main() {
  const donneesTest = {
    ...
    interets: ["Intelligence Artificielle", "Création de produits", "Management"],
