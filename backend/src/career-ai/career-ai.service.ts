...
const AUTOMATION_RISK_KEYWORDS: HeuristicKeyword[] = [
    {
        tokens: ['comptable', 'saisie', 'gestionnaire paie', 'assistant administratif'],
        score: 4,
        risks: [
...
const RESILIENCE_KEYWORDS: ResilienceKeyword[] = [
    {
        tokens: ['infirmier', 'infirmiere', 'aide soignant', 'medecin', 'sage femme'],
        score: 5,
        drivers: [
    ...
    {
        tokens: ['plombier', 'electricien', 'artisan', 'technicien terrain', 'mecanicien'],
        score: 5,
        drivers: [
...
const AUGMENTATION_KEYWORDS: AugmentationKeyword[] = [
    ...
    },
    {
        ...
        score: 5,
        adjacentRoles: [
            'Ingénieur intégration IA',
            'MLOps / AI operations junior',
        ],
    },
    {
...
const GENERIC_SKILLS: SkillRecommendation[] = [
    {
        ...
        priority: 'haute',
        reason: 'Comprendre ce que l’IA sait faire et ses limites est essentiel dans tous les métiers.',
    },
    {
        skill: 'Communication et collaboration',
        priority: 'moyenne',
        reason: 'Les métiers qui évoluent avec l’IA valorisent la coordination humaine.',
    },
    {
        skill: 'Maîtrise des outils numériques',
        priority: 'haute',
        reason: 'La productivité passe de plus en plus par des outils d’automatisation et d’assistance.',
    },
];
...
export class CareerAiService {
    analyzeCareer(input: AnalyzeCareerInput): CareerAiAnalysis {
        ...
        const augmentationPotential = this.sum(augmentationMatches.map((m) => m.score));

        const aiCompatibilityScore = this.clamp(
            Math.round(50 + resilienceStrength * 8 + augmentationPotential * 5 - automationPressure * 6),
            18,
            95,
        );
        ...
        const transformationImpact: TransformationImpact =
            transformationIntensity >= 8 ? 'élevé' : transformationIntensity >= 4 ? 'modéré' : 'faible';

        const timeHorizon: TimeHorizon =
            automationPressure >= 6 ? '0-3 ans' : transformationIntensity >= 3 ? '3-7 ans' : '7+ ans';
        ...
        const confidence = this.computeConfidence(
            automationMatches.length + resilienceMatches.length + augmentationMatches.length,
        );
        ...
        return {
            ...
            emergingAdjacentRoles:
                emergingAdjacentRoles.length > 0
                    ? emergingAdjacentRoles
                    : ['Coordinateur outils numériques', 'Spécialiste amélioration continue des processus'],
    ...
    private buildSkills(params: {
        ...
        if (params.automationPressure >= 5) {
            skills.unshift({
                ...
                priority: 'haute',
                reason: 'Le métier gagne en valeur quand vous pilotez, contrôlez et améliorez les sorties IA.',
            });
        }
        ...
        if (params.hasAugmentation) {
            skills.push({
                ...
                priority: 'moyenne',
                reason: 'Savoir guider les outils IA permet d’obtenir des résultats fiables plus rapidement.',
            });
        }
        ...
        if (params.transformationImpact === 'élevé') {
            skills.push({
                ...
                priority: 'haute',
                reason: 'Les outils et pratiques évoluent vite: la veille et l’adaptation deviennent clés.',
            });
        }
    ...
    private buildAdvice(params: {
        ...
        if (params.aiCompatibilityScore >= 70) {
            advice.push('Le métier reste pertinent avec l’IA: cible des rôles hybrides humain + outils IA.');
        } else {
            advice.push('Renforce un socle transférable pour garder plusieurs options d’évolution professionnelle.');
        }

        if (params.transformationImpact === 'élevé') {
            advice.push('Anticipe les changements dès maintenant avec une spécialisation complémentaire.');
        } else if (params.transformationImpact === 'modéré') {
            advice.push('Prévois une montée en compétences progressive sur les outils numériques du secteur.');
        } else {
            advice.push('Le cœur du métier évolue lentement, mais des outils IA amélioreront la productivité.');
        }

        if (params.resilienceDrivers.length > 0) {
            advice.push('Mise sur les compétences humaines à forte valeur: relationnel, jugement, créativité.');
        }

        if (params.automationRisks.length > 0) {
            advice.push('Identifie les tâches répétitives de ton métier pour te positionner sur les tâches à impact.');
        }
