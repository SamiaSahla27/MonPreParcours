...
export function OrientationIASession() {
  ...
  const loadIntroQuestions = useCallback(async () => {
    ...
    try {
      ...
      setQuestionError("Impossible de charger le questionnaire. Verifie ta connexion puis reessaie.");
    } finally {
          ...
          <AnimatePresence mode="wait">
            ...
            {phase === "chat" ? (
              <motion.section
                ...
                exit={{ opacity: 0, y: -12 }}
                transition={{ duration: 0.3 }}
                className="mx-auto flex h-[calc(100vh-150px)] w-full max-w-6xl flex-col rounded-3xl border border-indigo-200/60 bg-white/70 p-6 shadow-[0_20px_70px_rgba(92,65,188,0.14)] backdrop-blur-xl"
              >
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-indigo-600">
                      Voici ton plan d'action base sur tes reponses
                    </p>
                  </div>
                  <button
                    onClick={handleCloseSession}
                    className="rounded-full p-2 text-indigo-600 transition hover:bg-indigo-100"
                    aria-label="Archiver et fermer la session"
                  >
                ...
                <div className="flex-1 overflow-auto">
                  ...
                  <div ref={chatEndRef} className="pt-4" aria-hidden="true">
                    {assistantTyping ? (
                ...
                >
                  <input
                    ...
                    className="flex-1 rounded-2xl border border-indigo-200/70 bg-white px-4 py-3 text-sm text-indigo-950 outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-200/40"
                    aria-label="Message du chat"
                  />
                  <button
                    type="submit"
                    className="flex-shrink-0 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-indigo-700"
